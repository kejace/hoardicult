...
Evaluates relay schedules from boards.json and automatically opens/closes
relays based on time-of-day rules.
"""
...
class SchedulerService:
    """Background service that controls relays based on time schedules."""
    ...
    def __init__(
        ...
        # Set of (board_addr, relay_num) currently being controlled by scheduler
    ...
    async def stop(self) -> None:
        ...
        # Turn off all scheduler-controlled relays
    ...
    async def _tick(self) -> None:
        """Evaluate all schedules and issue relay commands as needed."""
        now = datetime.now()
        current_time = now.time()

        for config in self._board_configs:
            board_addr = config["board_addr"]
            schedules: dict[str, dict] = config.get("schedules", {})

            for relay_str, sched_data in schedules.items():
                relay_num = int(relay_str)
                schedule = _parse_schedule(sched_data)
                if schedule is None:
                    continue

                key = (board_addr, relay_num)
                should_on = _should_be_on(schedule, current_time)
    ...
    ) -> RelayScheduleInfo | None:
        """Return schedule info for a relay, or None if no schedule."""
        for config in self._board_configs:
            if config["board_addr"] != board_addr:
                continue
            schedules: dict[str, dict] = config.get("schedules", {})
            sched_data = schedules.get(str(relay_num))
            ...
            now = datetime.now()
            current_time = now.time()
            is_active = (board_addr, relay_num) in self._scheduler_active
            next_on, next_off = _next_on_off(schedule, current_time)
